...
class Point :
    ...
    def ShortestDistance(self) :
        return max(SIZE - 1 - self.x, SIZE - 1 - self.y)

    def EstimatedDistance(self) :
        return self.ShortestDistance() + self.covereddistance
...
while (solutionlist != []):
    ...
    if currentPoint.t == SIZE * SIZE - 1 :
        break
    r, l, u, d, ru, rd, lu, ld = currentPoint.ToRight(), currentPoint.ToLeft(), currentPoint.Up(), currentPoint.Down(), currentPoint.RightUp(), currentPoint.RightDown(), currentPoint.LeftUp(), currentPoint.LeftDown()
    ...
    if ru != None: solutionlist.append(ru)
    if rd != None: solutionlist.append(rd)
    if lu != None: solutionlist.append(lu)
    if ld != None: solutionlist.append(ld)

    solutionlist = sorted(solutionlist, key=lambda x:x.estimateddistance, reverse=True)
